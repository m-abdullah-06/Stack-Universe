...
export function HUD({ data, cockpitMode = false, setCockpitMode }: HUDProps) {
  ...
  const isOwner =
    session?.user && (session.user as any).login === data.username;
  ...
  const isCustomiseOpen = activePanel === "customise";
  const isHoroscopeOpen = activePanel === "horoscope";
        ...
        <div className="md:hidden">
          ...
          <div className="flex items-center justify-between px-3 py-2.5">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-space-cyan border-[1.5px] border-black flex items-center justify-center text-[5px] text-black font-bold">
                  ✓
                </div>
              </div>
              <div className="min-w-0">
                <p className="font-orbitron font-bold text-white text-[11px] leading-tight truncate">
                  @{data.username}
                </p>
                <p className="text-[9px] font-orbitron font-bold text-space-cyan">
                  {data.universeScore.toLocaleString()}{" "}
                  <span className="text-[7px] font-mono text-gray-500 font-normal">
                    PTS
                  </span>
                </p>
              </div>
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              ...
              <button
                onClick={() => setActivePanel(activePanel === 'giants' ? null : 'giants')}
                className="p-1.5 text-space-gold/70 hover:text-space-gold transition-colors"
                title="Leaderboard"
              >
                <svg
                  className="w-3.5 h-3.5"
          ...
          <div className="flex items-center justify-between px-3 pb-2.5 gap-2">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-orbitron text-space-gold font-bold">
                ★{data.totalStars.toLocaleString()}
              </span>
              <span className="text-[10px] font-orbitron text-white font-bold">
                {data.repos.length}{" "}
                <span className="text-[8px] font-mono text-gray-600 font-normal">
                  repos
                </span>
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              {isOwner && isClaimed && (
                <button
                  onClick={() => setActivePanel("customise")}
          ...
          <div className="flex items-center justify-around px-3 pb-3 border-t border-white/5 pt-2 bg-white/[0.02]">
            ...
            >
              <span className="text-[14px] group-hover:scale-110 transition-transform text-space-magenta">
                ⧉
              </span>
              <span className="text-[7px] font-mono text-space-magenta/60 uppercase group-hover:text-space-magenta transition-colors">
                Share
              </span>
            </button>
          </div>
        ...
        <div className="hidden md:block p-4 space-y-3 max-h-[calc(100vh-80px)] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-shrink-0 group">
              ...
              <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-space-cyan border-2 border-[#020205] flex items-center justify-center text-[7px] text-black font-bold">
                ✓
              </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-[10px] font-orbitron text-space-cyan font-bold leading-none">
                  {data.universeScore.toLocaleString()}
                </span>
            ...
            <div className="space-y-3">
              <label
                className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-all duration-300 group ${
                  isWeeklyDigestChecked
                    ? "bg-space-cyan/5 border-space-cyan/30 shadow-[0_0_15px_rgba(0,229,255,0.1)]"
              ...
              >
                <div className="relative">
                  ...
                  />
                  <div
                    className={`w-5 h-5 border-2 rounded transition-all ${isWeeklyDigestChecked ? "border-space-cyan bg-space-cyan/20" : "border-white/20"}`}
                  />
                  <div
                    className={`absolute inset-0 flex items-center justify-center transition-opacity ${isWeeklyDigestChecked ? "opacity-100" : "opacity-0"}`}
                  >
                    <svg
                      className="w-3 h-3 text-space-cyan"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <div className="flex-1">
                  <p
                    className={`text-[10px] font-orbitron font-bold transition-colors ${isWeeklyDigestChecked ? "text-space-cyan" : "text-white"}`}
                  >
                    Weekly Digest Email
                  </p>
                  <p className="text-[8px] font-mono text-gray-500">
                    Every Monday summary of your universe growth.
                  </p>
                </div>
              </label>
          ...
          <div className="border-t border-white/5 pt-3 space-y-2">
            <div className="flex justify-between items-baseline">
              ...
              <span className="text-sm font-orbitron font-bold text-space-cyan">
                {data.universeScore.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center gap-2 bg-space-cyan/5 border border-space-cyan/15 rounded-lg px-2.5 py-1.5">
              ...
              <span className="text-[9px] font-mono text-space-cyan tracking-wider">
                {data.distanceLabel}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white/[0.03] rounded-lg p-2 text-center">
              ...
              <p className="font-orbitron text-xs text-space-gold font-bold">
                ★{data.totalStars.toLocaleString()}
              </p>
            </div>
            <div className="bg-white/[0.03] rounded-lg p-2 text-center">
              ...
              <p className="font-orbitron text-xs text-white font-bold">
                {data.repos.length}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-stretch gap-2 h-10">
            ...
            {isOwner && isClaimed && (
              ...
              >
                <span className="text-[14px] group-hover:rotate-90 transition-transform duration-500">
                  ⚙️
                </span>
              </motion.button>
            )}
          ...
          <div className="space-y-3 pt-1">
            <p className="text-[8px] font-mono text-white/20 uppercase tracking-[0.2em] px-1 pt-2">
            ...
            <div className="relative group">
                  ...
                  if (e.key === "Enter") {
                    ...
                    try {
                      ...
                      if (json.matches && json.matches.length > 0) {
                        setQueriedPlanetNames(json.matches);
                        setQueryFeedback(
                          `${json.matches.length} matches identified`,
                        );
              ...
              <div className="absolute left-3.5 top-1/2 -translate-y-1/2 flex items-center gap-2">
                {isQuerying ? (
                  ...
                  <svg
                    className="w-3.5 h-3.5 text-white/20 group-focus-within:text-space-cyan transition-colors"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
